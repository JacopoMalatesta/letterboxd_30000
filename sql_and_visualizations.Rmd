...
## Countries by number of films in the database
...
SELECT UNNEST(STRING_TO_ARRAY(country, ';')) country,
       ...
       COUNT(*) :: NUMERIC / (SELECT COUNT(*) FROM info) "Ratio"
...
About 82% of the films in the database had either an American, French or Italian production. This result can be chalked up to: a) Hollywood's dominance of film industry; b) Mereghetti's strong interest in French culture; c) Mereghetti's Italian nationality.

</div>

<br><br>

## 14. Average runtime over time
...
## 1. Frequency distribution of ratings
...
## 2. Frequency of four-star rated films over time
...
## 3. Frequency of four-star rated films by decade
...
## 4. Frequency of four-star rated films by country
...
## 5. Top 5 countries by n. of four star rated films over time
...
## 6. Frequency of four-star rated films by genre
...
## 7. Frequency of four-star rated films by genre over time
...
# Most common country-genre combinations?

What are the most common country-genre combinations with rating = 8?

```{sql connection=con, output.var = "country_genre"}

WITH CTE AS ( SELECT UNNEST(STRING_TO_ARRAY(country, ';')) "Country",
...
SELECT "Country", "Genre", count :: INTEGER "Number of films", "Titles"
FROM CTE
...
```{r}
country_genre <- country_genre %>% mutate_if(is.character, utf8_encode)

datatable(country_genre, rownames = FALSE, filter="top", options = list(pageLength = 1, scrollX=T))
```
...
p <- country_genre %>% 
  ggplot(aes(x = Country, y = Genre, fill = `Number of films`)) +
  geom_tile() +
  scale_fill_gradient(low = "light blue", high = "dark blue", na.value = "white") +
  theme_classic() +
  labs(title = "Most common country-genre combinations for four-star rated films") +
...
## 8. Directors with most four-star-rated films
...
p <- four_rated_by_director %>% 
  ...
  ggplot(aes(x = count, y = director, fill = count)) +
  geom_col() +
  theme_classic() +
  scale_fill_gradient(low = "light blue", high = "dark blue", guide = "none") +
...
## 9. Directors with most four-star-rated films by decade
...
## 10. Directors with most four-star-rated films by genre
